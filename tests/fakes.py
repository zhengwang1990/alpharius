...
class FakeTradingClient:
    ...
    @staticmethod
    def _get_asset(symbol: str) -> trading.Asset:
        return trading.Asset(
            id=uuid.uuid4(),
    ...
    def get_all_assets(self, filter: trading.GetAssetsRequest):
        ...
        return [self._get_asset(symbol) for symbol in ['QQQ', 'SPY', 'DIA', 'TQQQ', 'GOOG', 'AAPL', 'MSFT']]
    ...
    def get(self, path: str, data: dict | None = None):
        assert path == '/account/portfolio/history', f'Unsupported path {path}'
        self.get_portfolio_history_call_count += 1
        time_interval = {'1D': 86400, '1H': 3600, '5Min': 300}[data['timeframe']]
        timestamps = [
            t
            for t in range(_to_timestamp(data['start']), _to_timestamp(data['end']), time_interval)
        ...
        return {'timestamp': timestamps, 'equity': equity}
