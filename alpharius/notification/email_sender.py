...
class EmailSender:
    ...
    def send_summary(self, data_client: data.DataClient):
        ...
        for position in positions:
            ...
            positions_html += (
                f'<tr><th scope="row">{position.symbol}</th>'
        ...
        for transaction in transactions:
            ...
            transactions_html += (
                f'<tr><th scope="row">{transaction.symbol}</th>'
        ...
        for i in range(len(history.equity)):
            history.equity[i] = (
                history.equity[i] - cash_reserve if history.equity[i] > cash_reserve else history.equity[i]
            )
        ...
        for symbol in market_symbols:
            df = data_client.get_data(
                symbol, pd.Timestamp(historical_date[0]), pd.Timestamp(historical_date[-1]), data.TimeInterval.DAY
            )
        ...
        account_html = (
            f'<tr><th {row_style}>Equity</th><td>{account_equity:.2f}</td></tr>'
        ...
        for symbol in market_symbols:
            ...
            account_html += (
                f'<tr><td style="border: none; padding: 0.5rem;">{symbol}</td>'
        ...
        plt.xticks(range(len(historical_date)), [date.strftime('%m-%d') for date in historical_date], **text_kwargs)
        plt.xlabel('Date', **text_kwargs)
        plt.ylabel('Normalized Value', **text_kwargs)
        plt.yticks(fontname='monospace')
        plt.grid(linestyle='--', alpha=0.5)
        plt.legend(ncol=len(market_symbols) + 1, bbox_to_anchor=(0, 1), loc='lower left', prop=text_kwargs)
        ax = plt.gca()
        ...
        message.attach(
            text.MIMEText(
                html_template.format(
                    account_html=account_html, transactions_html=transactions_html, positions_html=positions_html
                ),
                'html',
            )
        )
...
def main():
    ...
    parser.add_argument(
        '-m', '--mode', help='Running mode. Can be summary or alert.', required=True, choices=['summary', 'alert']
    )
    parser.add_argument('--error_message', help='Error message. Only used in alert mode.')
