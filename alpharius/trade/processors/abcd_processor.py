...
class AbcdProcessor(Processor):
    ...
    def _open_position(self, context: Context) -> Optional[ProcessorAction]:
        ...
        if datetime.time(10, 30) <= t <= datetime.time(15, 30) and context.current_price > context.prev_day_close:
            if context.symbol == 'TQQQ':
                return self._open_long_position(context, 0.7, 0.4)
            else:
                return self._open_long_position(context, 1, 0.6)
        if (
            datetime.time(12, 0) <= t <= datetime.time(15, 30)
    ...
    def _close_position(self, context: Context) -> Optional[ProcessorAction]:
        ...
        is_close = (
            take_profit
            or context.current_time >= position['entry_time'] + datetime.timedelta(minutes=40)
